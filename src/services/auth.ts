...
export const login = async (
  ...
  try {
    await check("email", "Email is not valid").isEmail().run(req); //이메일인지 검사하는 부분
    await check("password", "Password cannot be blank") //비번인지 검사하는 부분
      .isLength({ min: 1 })
      .run(req);
    await sanitize("email") //email 정규화하는 과정같은데?
      .normalizeEmail({ gmail_remove_dots: false })
      .run(req);

    const errors: Result<ValidationError> = validationResult(req);

    //에러가 있으면 여기서 응답을해준다고 되어있는데.. 여기서 응답말고 routes에서 응답해줘야되는거아닌가?
    if (!errors.isEmpty()) {
...
export const signup = async (
  ...
  try {
    ...
    await check("password", "비밀번호는 최소 4글자 이상이어야 합니다.")
      .isLength({ min: 4 })
      .run(req);
    await check("confirmPassword", "재입력 비밀번호가 일치하지 않습니다.")
